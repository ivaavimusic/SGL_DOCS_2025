...
<Note>Install `sgl` from the official open-source release (see [Node setup](/cloud/provide/node-setup)). Chat models use a local inference runtime such as llama-server and a GGUF model file. System One models can use a local loopback sidecar.</Note>
...
| `--systemone-sidecar-url` | — | Loopback URL for a System One sidecar such as Laya or CLM |
...
`--systemone-sidecar-url` must point to loopback (`127.0.0.1`, `localhost`, or `::1`). The sidecar is not exposed to the internet; the node receives Grid jobs and forwards them locally.
