...
<Note>
  This skill is production-ready and has been tested with real USDC payments on Base network.
</Note>
...
<Tabs>
  <Tab title="Consumer Mode">
    ...
    | Script | Purpose |
    |--------|---------|
    | `pay_base.py` | Pay for endpoints on Base network (100% reliable) |
    | `pay_solana.py` | Pay for endpoints on Solana network (with retry) |
  ...
  <Tab title="Provider Mode">
    ...
    | Script | Purpose |
    |--------|---------|
    | `create_endpoint.py` | Deploy new monetized endpoint ($5 fee) |
