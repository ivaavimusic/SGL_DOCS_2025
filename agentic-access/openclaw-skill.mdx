...
<Note>
  Current release: **v1.4.0**. ERC-8004 is now wallet-first by default and the skill includes agent-side discovery + management scripts for listing bindable endpoints, listing owned agents, registering with rich metadata, and updating agents after mint.
</Note>
