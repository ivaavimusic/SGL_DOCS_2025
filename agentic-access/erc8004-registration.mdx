...
Use the update routes for endpoint links, metadata changes, and marketplace visibility. The legacy `PATCH /agent/erc8004/:agentId` path remains available during migration.
...
<Note>
This is the deprecated x402-backed path. Use it only if you still need the old worker-managed registration flow during migration.
</Note>
...
// 4. Call /agent/erc8004/prepare with Authorization: Bearer <token>
// 5. Wallet sends the prepared transaction
// 6. Call /agent/erc8004/finalize to index the agent
```

## Registration Fee

<Info>
Wallet-first registration has no platform fee by default. The agent wallet pays chain gas directly. x402 is still useful for paid endpoints and legacy registration flows.
</Info>
